...
module Examples.Poseidon2.RoundConst where
...
initialRoundConstsℕ : Vec (Vec ℕ 3) 4
initialRoundConstsℕ =
...
finalRoundConstsℕ : Vec (Vec ℕ 3) 4
finalRoundConstsℕ =
...
internalRoundConstsℕ : Vec ℕ 56
internalRoundConstsℕ =
...
internalRoundConst : (ℕ -> A) -> Fin 56 -> A
internalRoundConst f j = f (lookup internalRoundConstsℕ j)
...
initialRoundConst : (ℕ -> A) -> Fin 4 -> A × A × A
initialRoundConst f j = (f x , f y , f z) where

  triple : Vec ℕ 3
  triple = lookup initialRoundConstsℕ j

  x = lookup triple f0
  y = lookup triple f1
  z = lookup triple f2
  
finalRoundConst : (ℕ -> A) -> Fin 4 -> A × A × A
finalRoundConst f j = (f x , f y , f z) where

  triple : Vec ℕ 3
  triple = lookup finalRoundConstsℕ j
